...
// deadlines do, so a single unreachable or misbehaving upstream cannot pin a
// worker thread on a saturated pool.
...
// idle before we assume the server has closed it and rebuild (via resumption)
// instead of reusing a probably-dead socket. Seeded from measured values (see
// seed_idle_ms) and adapted at runtime, clamped to [MIN, MAX] with a slack band
// for hysteresis.
...
// store, the fail-closed verify mode and the client session cache for all
// upstreams; each connection draws its own SSL object from it. OpenSSL seeds its
// own RNG, so no explicit DRBG is wired up here.
...
// SYN) would pin the worker for the kernel's full SYN timeout (~2 min). Connect
...
static void conn_teardown(struct tls_conn *c)
{
	...
	{
		if(c->connected)
			SSL_shutdown(c->ssl); // best-effort notify
		SSL_free(c->ssl);
		c->ssl = NULL;
	}
	if(c->fd >= 0)
                         ...
                         uint64_t deadline, enum hs_outcome *outcome)
{
	...
	*outcome = HS_FRESH_COLD;
	...
	// receive window fills) would block SSL_read()/SSL_write() forever.
	...
	// or the deadline passes. A verification failure returns a hard error here,
	// which is exactly the fail-closed behaviour.
	int rc;
                       ...
                       uint8_t *answer, size_t answer_sz, uint64_t deadline)
{
	...
	for(;;)
	{
		...
		// fills without ever hitting the WANT_READ deadline below.
                          ...
                          uint8_t *answer, size_t answer_sz)
{
	...
	// stalled connect, handshake, read or write cannot pin a worker forever. A
	// pooled connection the upstream closed while idle is transparently rebuilt
	// once; a second failure is real and we fail closed.
...
// Without TLS there is no TLS client; encrypted upstreams are unavailable and
// every exchange fails closed. The config layer refuses to enable them. These
// trivial stubs are const-folding candidates, so GCC raises
// -Wsuggest-attribute=const; the attribute cannot be applied (it conflicts with
// tls_pool_new()'s malloc attribute), so silence the suggestion here. clang does
// not have this warning, hence the GCC-only guard.
